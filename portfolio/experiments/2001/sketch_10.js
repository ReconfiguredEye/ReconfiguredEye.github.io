...
function drawFloorAndCeiling() {
  ...
  drawPlaneWithBeams(0, floorY-2, zed, cols, rows, blockWidth, blockHeight, gap, true);
  drawPlaneWithBeams(0, ceilingY+4, zed, cols, rows, blockWidth, blockHeight, gap, false);
}
...
function drawPlaneWithBeams(x, y, z, cols, rows, blockWidth, blockHeight, gap, isFloor) {
    ...
    fill(0, 0, 5); 
    let beamWidth = 150;
    rect(-beamWidth / 2, -rows * (blockHeight + gap) / 2, beamWidth, rows * (blockHeight + gap));
